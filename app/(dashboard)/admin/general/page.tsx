// app/(dashboard)/admin/general/page.tsx
...
const LoadingSkeleton = () => (
  <div className="wander-general-container">
    <div className="wander-general-header">
      <div>
        <span className="wander-breadcrumb">Services / General</span>
        <h2>Loading services...</h2>
      </div>
    </div>
    <div className="wander-general-loading">
...
export default function GeneralServiceListPage() {
  ...
  const getPriceTypeLabel = (type: string) => {
    const typeMap: Record<string, { label: string; color: string }> = {
      'cotizacion': { label: 'Quote', color: '#f59e0b' },
      'fijo': { label: 'Fixed', color: '#2563eb' },
    };
    ...
    <div className="wander-general-container">
      ...
      <header className="wander-general-header">
        <div>
          ...
          <p className="wander-general-subtitle">
      ...
      {/* Grid */}
      <div className="wander-general-grid">
        {currentItems.length === 0 ? (
          <div className="wander-general-empty">
            <span className="wander-empty-icon">
              <FiFileText size={48} />
            </span>
            <p>No services found</p>
            <span className="wander-empty-desc">No general services registered yet</span>
          </div>
        ) : (
          currentItems.map((service) => (
            <div key={service.id} className="wander-general-card">
              <div className="wander-general-image">
                {service.principal_image ? (
                  <img src={service.principal_image} alt={service.name} />
                ) : (
                  <div className="wander-general-image-placeholder">
                    <FiFileText size={48} />
                  </div>
                )}
                <div className="wander-general-status-badge">
                  {getStatusBadge(service.status)}
                </div>
              </div>
              
              <div className="wander-general-info">
                <h3 className="wander-general-title">{service.name}</h3>
                
                <div className="wander-general-details">
                  <span>
                ...
                <div className="wander-general-price">
                  {service.price && service.price_type?.toLowerCase() === 'fijo' ? (
                    <>
                      <FiDollarSign size={16} />
                      {formatCurrency(service.price)}
                    </>
                  ) : service.price_type?.toLowerCase() === 'cotizacion' ? (
                    <>
                      <FiHelpCircle size={16} style={{ color: '#f59e0b' }} />
                    ...
                    <>
                      <FiDollarSign size={16} style={{ color: '#717171' }} />
                      <span style={{ fontSize: '14px', fontWeight: 400, color: '#717171' }}>
                        Price not set
                      </span>
                    </>
                  )}
                ...
                {service.descripcion && (
                  <p className="wander-general-description">
              ...
              <div className="wander-general-actions">
                <Link 
                  href={`/admin/general/${service.id}`}
                  className="wander-general-action-btn details"
                >
                  <FiEye size={14} />
                  Details
                </Link>
      ...
      {/* Paginación */}
      {totalPages > 1 && (
        <div className="wander-general-pagination">
