// app/(dashboard)/admin/health/[id]/page.tsx
...
import { getHealthDetail, Health, deleteHealthService, uploadGallery, changePrincipalImage } from '@/app/lib/api/healthAdmin';
import {
  FiArrowLeft,
  FiEdit2,
  FiTrash2,
...
import ModalEditHealth from '@/app/(dashboard)/admin/components/ModalEditHealth';
import './detail.css';
...
interface HealthDetail extends Health {
...
const LoadingSkeleton = () => (
  <div className="wander-detail-container">
    <div className="wander-detail-loading">
      ...
      <p>Loading health service details...</p>
    </div>
  </div>
);
...
export default function HealthDetailPage() {
  ...
  const [service, setService] = useState<HealthDetail | null>(null);
  ...
  const loadService = useCallback(async () => {
    ...
    try {
      ...
      setError((err instanceof Error ? err.message : undefined) || 'Error loading health service details');
    } finally {
      setIsLoading(false);
    }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      console.error('❌ Error eliminando servicio de health:', err);
      setToast({
    ...
    return (
      <div className="wander-detail-container">
        <div className="wander-detail-error">
          <h3>⚠️ Error loading health service</h3>
          <p>{error}</p>
