// components/YachtInvoicePDF.tsx
'use client';

import jsPDF from 'jspdf';
import autoTable from 'jspdf-autotable';

export interface YachtInvoicePDFData {
  id: number;
  title: string;
  price: string;
...
export async function generateYachtInvoicePDF(invoice: YachtInvoicePDFData): Promise<void> {
  const doc = new jsPDF({
  ...
  doc.text(invoice.yacht_name, margin, yPos + 7);
  ...
  doc.setFont('helvetica', 'bold');
  doc.setTextColor('#000000');
  doc.text(invoice.yacht_name, margin, yPos);

  doc.setFontSize(12);
  doc.setFont('helvetica', 'normal');
  doc.setTextColor('#717171');
  doc.text(`(${invoice.lenght} ft)`, margin + 70, yPos);

  doc.setFontSize(11);
  doc.setTextColor('#717171');
  doc.text(`ID: ${invoice.yacht_id}`, margin, yPos + 6);
  ...
  doc.save(`Invoice_${invoice.id}_${invoice.yacht_name.replace(/\s+/g, '_')}.pdf`);
}
