// app/(dashboard)/admin/experiences/[id]/page.tsx
...
import { getExperienceDetail, Experience, deleteExperience, uploadGallery, changePrincipalImage } from '@/app/lib/api/experienceAdmin';
import {
  FiArrowLeft,
  FiEdit2,
  FiTrash2,
...
import ModalEditExperience from '@/app/(dashboard)/admin/components/ModalEditExperience';
import './detail.css';
...
export default function ExperienceDetailPage() {
  ...
  const loadExperience = useCallback(async () => {
    ...
    try {
      ...
      setError((err instanceof Error ? err.message : undefined) || 'Error loading experience details');
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    ...
    loadExperience();
  }, [isAuthenticated, isChecking, loadExperience, router, checkAuth]);
  ...
  const goToPreviousImage = () => {
    if (!experience?.galeria) return;
    setCurrentImageIndex(prev => 
      prev === 0 ? experience.galeria!.length : prev - 1
    );
  };
  ...
  const goToNextImage = () => {
    if (!experience?.galeria) return;
    setCurrentImageIndex(prev => 
      prev === experience.galeria!.length ? 0 : prev + 1
    );
  };
    ...
    return (
      <div className="wander-detail-container">
        <div className="wander-detail-error">
          <h3>⚠️ Error loading experience</h3>
          <p>{error}</p>
  ...
  const allImages = experience.galeria 
    ? [experience.principal_image, ...experience.galeria.map(g => g.url)].filter(Boolean)
    : [experience.principal_image].filter(Boolean);
  ...
  return (
    <div className="wander-detail-container">
      ...
      <ConfirmDialog
        ...
        message={`Are you sure you want to delete "${experience.name}"? This action cannot be undone.`}
        confirmText="Delete"
        cancelText="Cancel"
        type="danger"
      ...
      <ModalChangeImage
        ...
        itemId={experience.id}
        itemName={experience.name}
        currentImage={experience.principal_image}
      ...
      <div className="wander-detail-header">
        <div className="wander-detail-header-left">
          ...
          <div>
            ...
            <h1 className="wander-detail-title">{experience.name}</h1>
          </div>
        </div>
        ...
        <div className="wander-detail-grid">
          ...
          <div className="wander-detail-image-section">
            <div className="wander-detail-main-image">
              {experience.principal_image ? (
                <img 
                  src={experience.principal_image} 
                  alt={experience.name}
            ...
            {allImages.length > 1 && (
              <div className="wander-detail-gallery">
                {allImages.map((img, index) => (
                  ...
                  >
                    <img src={img} alt={`${experience.name} - ${index + 1}`} />
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="wander-detail-info-section">
            <div className="wander-detail-info-card">
              <h3 className="wander-detail-info-title">Experience Information</h3>
              ...
              <div className="wander-detail-info-grid">
                ...
                <div className="wander-detail-info-item">
                  ...
                  <span>
                    ...
                    {experience.capacity_min || 0} people
                  </span>
                </div>
                <div className="wander-detail-info-item">
                  ...
                  <span>
                    <FiUsers size={14} style={{ marginRight: '4px' }} />
                    {experience.capacity_max || 0} people
                  </span>
                </div>
                <div className="wander-detail-info-item">
                ...
                <div className="wander-detail-info-item">
                  ...
                  <span>
                    {experience.pet_friendly ? (
                      <FiHeart size={14} style={{ marginRight: '4px', color: '#ec4899' }} />
                    ) : (
                      <span style={{ color: '#717171' }}>No</span>
                    )}
                    {experience.pet_friendly ? 'Yes' : 'No'}
                  </span>
                </div>
            ...
            <div className="wander-detail-meta-card">
              ...
              <div className="wander-detail-meta-item">
                <FiClock size={14} />
                <span>Updated: {formatDate(experience.updated_at)}</span>
              </div>
            </div>
          </div>
        </div>
