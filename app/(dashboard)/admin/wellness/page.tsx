// app/(dashboard)/admin/wellness/page.tsx
...
import { getWellnessServices, Wellness, createWellnessService, deleteWellnessService } from '@/app/lib/api/wellnessAdmin';
import {
  FiPlus,
  FiRefreshCw,
  FiEye,
...
import { CreateWellnessForm } from '@/app/(dashboard)/admin/components/CreateWellnessForm';
  ...
  <div className="wander-wellness-container">
    <div className="wander-wellness-header">
      <div>
        ...
        <h2>Loading wellness services...</h2>
      </div>
    </div>
...
export default function WellnessListPage() {
  const router = useRouter();
  const { token, isChecking, isAuthenticated, checkAuth } = useAuth();
  
  const [services, setServices] = useState<Wellness[]>([]);
  ...
  const loadServices = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      console.error('❌ Error cargando servicios de bienestar:', err);
      setError((err instanceof Error ? err.message : undefined) || 'Error loading wellness services');
    } finally {
      setIsLoading(false);
    }
  ...
  const handleCreateService = async (data: { name: string }) => {
    ...
    try {
      ...
      console.error('❌ Error creando servicio de bienestar:', err);
      setToast({
        message: (err instanceof Error ? err.message : undefined) || 'Error creating wellness service',
        type: 'error'
      });
    } finally {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      console.error('❌ Error eliminando servicio de bienestar:', err);
      setToast({
    ...
    return (
      <div className="wander-wellness-container">
        ...
        <div className="wander-error-state">
          <h3>⚠️ Error loading wellness services</h3>
          <p>{error}</p>
    ...
    <div className="wander-wellness-container">
      ...
      <header className="wander-wellness-header">
        ...
        <div className="wander-wellness-actions">
          ...
          <Link
            href="/admin/accounting?servicio_tipo=wellness"
            className="wander-btn-secondary"
          >
            <FiBarChart2 size={16} />
            Transactions
      ...
      <div className="wander-wellness-grid">
        {currentItems.length === 0 ? (
          <div className="wander-wellness-empty">
            ...
            <p>No wellness services found</p>
            <span className="wander-empty-desc">No wellness services registered yet</span>
          </div>
        ) : (
          currentItems.map((service) => (
            <div key={service.id} className="wander-wellness-card">
              <div className="wander-wellness-image">
                {service.principal_image ? (
                  <img src={service.principal_image} alt={service.name} />
                ) : (
                  <div className="wander-wellness-image-placeholder">
                    <FiSun size={48} />
                  </div>
                )}
              ...
              <div className="wander-wellness-info">
                <h3 className="wander-wellness-title">{service.name}</h3>
                <p className="wander-wellness-meta">
              ...
              <div className="wander-wellness-actions">
                <Link 
                  href={`/admin/wellness/${service.id}`}
                  className="wander-wellness-action-btn details"
                >
                  <FiEye size={14} />
                  Details
                </Link>
                <button 
                  ...
                  className="wander-wellness-action-btn delete"
                >
                  <FiTrash2 size={14} />
                  Delete
                </button>
      ...
      <Modal
        ...
        title="Create New Wellness Service"
        maxWidth="640px"
      >
        <CreateWellnessForm
