// app/(dashboard)/admin/transport/[id]/page.tsx
...
import { getTransportDetail, Transport, uploadGallery, changePrincipalImage } from '@/app/lib/api/transportAdmin';
import {
  FiArrowLeft,
  FiEdit2,
  FiTrash2,
...
import ModalEditTransport from '@/app/(dashboard)/admin/components/ModalEditTransport';
import { deleteTransport } from '@/app/lib/api/transportAdmin';
import './detail.css';
...
interface TransportDetail extends Transport {
...
export default function TransportDetailPage() {
  ...
  const loadTransport = useCallback(async () => {
    ...
    try {
      ...
      setError((err instanceof Error ? err.message : undefined) || 'Error loading transport details');
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    ...
    loadTransport();
  }, [isAuthenticated, isChecking, loadTransport, router, checkAuth]);
  ...
  const goToPreviousImage = () => {
    if (!transport?.galeria) return;
    setCurrentImageIndex(prev => 
      prev === 0 ? transport.galeria!.length - 1 : prev - 1
    );
  };
  ...
  const goToNextImage = () => {
    if (!transport?.galeria) return;
    setCurrentImageIndex(prev => 
      prev === transport.galeria!.length - 1 ? 0 : prev + 1
    );
  };
    ...
    return (
      <div className="wander-detail-container">
        <div className="wander-detail-error">
          <h3>⚠️ Error loading transport</h3>
          <p>{error}</p>
  ...
  const allImages = transport.galeria 
    ? [transport.principal_image, ...transport.galeria.map(g => g.url)].filter(Boolean)
    : [transport.principal_image].filter(Boolean);
  ...
  return (
    <div className="wander-detail-container">
      ...
      <ConfirmDialog
        ...
        message={`Are you sure you want to delete "${transport.name}"? This action cannot be undone.`}
        confirmText="Delete"
        cancelText="Cancel"
        type="danger"
      ...
      <ModalChangeImage
        ...
        itemId={transport.id}
        itemName={transport.name}
        currentImage={transport.principal_image}
      ...
      {isLightboxOpen && allImages.length > 0 && (
        <div className="wander-lightbox-overlay" onClick={closeLightbox}>
          ...
          <img 
            ...
            alt={`${transport.name} - ${currentImageIndex + 1}`}
      ...
      <div className="wander-detail-header">
        <div className="wander-detail-header-left">
          ...
          <div>
            ...
            <h1 className="wander-detail-title">{transport.name}</h1>
          </div>
        </div>
        ...
        <div className="wander-detail-grid">
          ...
          <div className="wander-detail-image-section">
            <div className="wander-detail-main-image">
              {transport.principal_image ? (
                <img 
                  src={transport.principal_image} 
                  alt={transport.name}
            ...
            {allImages.length > 1 && (
              <div className="wander-detail-gallery">
                {allImages.map((img, index) => (
                  ...
                  >
                    <img src={img} alt={`${transport.name} - ${index + 1}`} />
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="wander-detail-info-section">
            <div className="wander-detail-info-card">
              <h3 className="wander-detail-info-title">Vehicle Information</h3>
              
              <div className="wander-detail-info-grid">
                <div className="wander-detail-info-item">
                  <label>Brand</label>
                  <span>{transport.brand || '—'}</span>
                </div>
                <div className="wander-detail-info-item">
                  <label>Model</label>
                ...
                <div className="wander-detail-info-item">
                  ...
                  <span className="wander-detail-price">
                    ...
                    {formatCurrency(transport.price_hour)}
                  </span>
                </div>
              </div>

              {transport.kit && (
                <div className="wander-detail-kit">
                  <FiGift size={16} />
                  <span>Kit: {transport.kit}</span>
                </div>
              )}
            ...
            <div className="wander-detail-meta-card">
              <div className="wander-detail-meta-item">
                <FiCalendar size={14} />
                <span>Created: {formatDate(transport.created_at)}</span>
              </div>
              <div className="wander-detail-meta-item">
                <FiClock size={14} />
                <span>Updated: {formatDate(transport.updated_at)}</span>
              </div>
            </div>
          </div>
        </div>
