...
import { useRouter } from 'next/navigation'; // Hook oficial para navegación fluida[cite: 7]
import { loginUser } from '../../lib/api'; // Importación centralizada del servicio[cite: 7]
import './login.css';

export default function LoginPage() {
  const router = useRouter();
  
  // Estados para controlar los valores del formulario[cite: 7]
  const [username, setUsername] = useState('');
  const [password, setPassword] = useState('');
  
  // Estados para el flujo de la petición[cite: 7]
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirección exacta al dashboard del subdominio de la app[cite: 7]
  ...
  return (
    <main className="login-split-page">
      ...
      {/* COLUMNA IZQUIERDA: FORMULARIO MINIMALISTA[cite: 7] */}
      <section className="login-form-side">
        <div className="login-box-container">
          ...
          <form onSubmit={handleLoginSubmit} className="wander-login-form">
            ...
            {/* Mostrar mensaje de error si la petición falla[cite: 7] */}
            {errorMessage && (
              <div style={{
                padding: '12px 14px',
      ...
      {/* COLUMNA DERECHA: IMAGEN CINEMÁTICA INMERSIVA[cite: 7] */}
